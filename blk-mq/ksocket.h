...
extern ssize_t ksend(ksocket_t socket, const void *buffer, size_t length,
                     int flags);
extern ssize_t krecvfrom(ksocket_t socket, void *buffer, size_t length,
                         int flags, struct sockaddr *address, int *address_len);
extern ssize_t ksendto(ksocket_t socket, void *message, size_t length,
                       int flags, const struct sockaddr *dest_addr,
                       int dest_len);

extern int kgetsockname(ksocket_t socket, struct sockaddr *address,
                        int *address_len);
extern int kgetpeername(ksocket_t socket, struct sockaddr *address,
                        int *address_len);
extern int ksetsockopt(ksocket_t socket, int level, int optname, void *optval,
                       int optlen);
extern int kgetsockopt(ksocket_t socket, int level, int optname, void *optval,
                       int *optlen);
